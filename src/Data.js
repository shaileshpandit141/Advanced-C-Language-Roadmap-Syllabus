export default [
    {
        id: 1,
        heading: 'Introduction to Programming',
        subHeadings: [
            'Understand basic programming concepts(variables, data types, operators).',
            'Learn about input and output operations.',
        ]
    },
    {
        id: 2,
        heading: 'Control Flow',
        subHeadings: [
            'Study decision - making structures(if statements, switch).',
            'Explore loop structures(for, while, do-while).',
        ]
    },
    {
        id: 3,
        heading: 'Functions',
        subHeadings: [
            'Learn how to define and use functions.',
            'Understand the concept of scope and lifetime.',
        ]
    },
    {
        id: 4,
        heading: 'Arrays and Strings',
        subHeadings: [
            'Work with one - dimensional and multidimensional arrays.',
            'Understand strings and string manipulation.',
        ]
    },
    {
        id: 5,
        heading: 'Pointers',
        subHeadings: [
            'Introduction to pointers and memory management.',
            'Learn about pointer arithmetic.',
        ]
    },
    {
        id: 6,
        heading: 'Structures and Unions',
        subHeadings: [
            'Define and use structures and unions.',
            'Explore nested structures.,',
        ]
    },
    {
        id: 7,
        heading: 'File Handling',
        subHeadings: [
            'Learn to read from and write to files.',
            'Understand file positioning.',
        ]
    },
   
    {
        id: 8,
        heading: 'Dynamic Memory Allocation',
        subHeadings: [
            'Explore dynamic memory allocation using malloc, calloc, realloc, and free.',
            'Understand memory leaks and how to avoid them.',
        ]
    },
    {
        id: 9,
        heading: 'Advanced Data Types',
        subHeadings: [
            'Learn about enums and typedef.',
            'Understand the use of bit - fields.',
        ]
    },
    {
        id: 10,
        heading: 'Advanced Pointers',
        subHeadings: [
            'Study pointers to functions.',
            'Explore function pointers.',
        ]
    },
    {
        id: 11,
        heading: 'Preprocessor Directives',
        subHeadings: [
            'Learn about macros and conditional compilation.',
            'Understand include guards.',
        ]
    },
    {
        id: 12,
        heading: 'Data Structures',
        subHeadings: [
            'Study linked lists, stacks, queues, and trees.',
            'Implement common algorithms using these structures.',
        ]
    },
    {
        id: 13,
        heading: 'Advanced File Handling',
        subHeadings: [
            'Explore binary file I / O.',
            'Understand random access in files.',
        ]
    },
    {
        id: 14,
        heading: 'Multithreading and Concurrency',
        subHeadings: [
            'Learn the basics of multithreading.',
            'Understand synchronization and locks.',
        ]
    },
    {
        id: 14,
        heading: 'Networking and Socket',
        subHeadings: [
            'Introduction to socket programming.',
            'Learn to create basic networked applications.',
        ]
    },
    {
        id: 15,
        heading: 'Understanding the Standard Template Library (STL)',
        subHeadings: [
            'Explore C++ Standard Template Library (though C does not officially support it, understanding it can be beneficial).',
        ]
    },
    {
        id: 16,
        heading: 'Debugging and Optimization',
        subHeadings: [
            'Learn to use debugging tools.',
            'Understand common optimization techniques.',
        ]
    },
    {
        id: 17,
        heading: 'Project Work',
        subHeadings: [
            'Undertake larger projects to apply your knowledge.',
            'Collaborate on open - source projects if possible.',
        ]
    },
    {
        id: 18,
        heading: 'Read Code',
        subHeadings: [
            'Analyze existing code to understand different coding styles.',
            'Contribute to open - source projects.',
        ]
    },
    {
        id: 19,
        heading: 'Explore Advanced Topics',
        subHeadings: [
            'Learn about advanced topics such as memory - mapped files, signal handling, etc.',
            'Explore the latest C standards and features.',
        ]
    },
    {
        id: 20,
        heading: 'Benefit From',
        subHeadings: [
            'Remember to practice regularly, build small projects, and seek help when needed. Online coding platforms, forums, and communities can provide valuable support. Additionally, refer to authoritative C programming books for in-depth understanding and keep abreast of the C language developments.',
        ]
    },
]